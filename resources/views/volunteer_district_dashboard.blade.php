...
<section class="max-w-4xl mx-auto my-8 space-y-6 px-4">
    ...
    <div class="bg-white p-5 rounded-xl shadow border border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
            <span class="text-xs font-bold text-brandOrange uppercase tracking-wider block">District President Workspace</span>
            <h2 class="text-lg font-black text-brandGray mt-0.5">District Jurisdiction: {{ session('auth_volunteer_locality') }}</h2>
        </div>
        <div class="flex items-center gap-3">
            <span class="text-xs font-bold text-gray-400 uppercase">District Pres ID: {{ session('auth_volunteer_code') }}</span>
    ...
    <!-- 2. Live Analytics Strategic District Indicator Cards -->
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white p-4 rounded-xl shadow border-b-4 border-brandOrange text-center">
            ...
            <span class="text-[10px] font-black text-gray-400 uppercase tracking-wider block">Total Assembly Segments</span>
            <span class="text-xl font-black text-brandGray block mt-0.5">{{ $totalDistrictAssemblies ?? 10 }} Assemblies</span>
        </div>
        <div class="bg-white p-4 rounded-xl shadow border-b-4 border-orange-500 text-center">
            <span class="text-xl block mb-1">👥</span>
            <span class="text-[10px] font-black text-gray-400 uppercase tracking-wider block">Total District Members</span>
            <span class="text-xl font-black text-brandGray block mt-0.5">{{ $totalDistrictMembers ?? 0 }} Members</span>
        </div>
        <div class="bg-white p-4 rounded-xl shadow border-b-4 border-green-500 text-center">
            <span class="text-xl block mb-1">🎁</span>
            <span class="text-[10px] font-black text-gray-400 uppercase tracking-wider block">Total District Seva</span>
            <span class="text-xl font-black text-green-600 block mt-0.5">{{ $totalDistrictBenefits ?? 0 }} Benefits</span>
        </div>
    </div>
    ...
    <div class="bg-white p-5 rounded-xl shadow border border-orange-100 space-y-4">
        <h3 class="text-xs font-black text-brandOrange uppercase tracking-wider border-b border-orange-100 pb-2 flex items-center gap-1"><span>🛡️</span> District Service Audit Control Desk</h3>
        
        <form action="/volunteer/dashboard/district" method="GET" class="flex flex-col sm:flex-row gap-3">
            ...
            <div class="mt-4 pt-2 border-t border-dashed border-gray-200 space-y-4">
                @if(isset($searchedAuditMember))
                    <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 text-xs text-brandGray">
                        ...
                        <p class="font-semibold text-gray-500 uppercase">District Address Mapped: <strong class="text-brandGray">{{ $searchedAuditMember->grama_panchayat }}, {{ $searchedAuditMember->mandal }}, {{ $searchedAuditMember->assembly_segment }}</strong></p>
                    </div>
    ...
    <!-- 4. Section A: Assembly Presidents (District Tier 3 Force) Directory Table -->
    <div class="bg-white p-5 rounded-xl shadow border border-gray-100 space-y-3">
        <h3 class="text-xs font-black text-brandGray uppercase tracking-wider border-b border-gray-100 pb-2">📂 Assembly Presidents District Tier-3 Force</h3>
            ...
            <table class="w-full text-left border-collapse text-xs text-brandGray">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200 text-gray-400 font-bold uppercase">
                        <th class="p-2.5">Assembly Jurisdiction</th>
                        <th class="p-2.5">Assembly Pres ID</th>
                        <th class="p-2.5">President Name</th>
                        <th class="p-2.5">Contact Phone</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @if(isset($assemblyPresidents) && count($assemblyPresidents) > 0)
                        @foreach($assemblyPresidents as $ap)
                            <tr class="hover:bg-gray-50 font-medium">
                                <td class="p-2.5 font-bold text-brandOrange uppercase">{{ $ap->locality }}</td>
                                <td class="p-2.5 tracking-wider">{{ $ap->volunteer_id }}</td>
                                <td class="p-2.5 uppercase text-brandDarkGray">{{ $ap->full_name ?? 'ASSIGNED ASSEMBLY PRES' }}</td>
                                <td class="p-2.5">+91 {{ $ap->phone }}</td>
                            </tr>
                        @endforeach
                    @else
                        <tr>
                            <td colspan="4" class="p-4 text-center text-gray-400 font-medium">No active assembly presidents mapped inside this district boundaries yet.</td>
                        </tr>
                    @endif
                </tbody>
            </table>
